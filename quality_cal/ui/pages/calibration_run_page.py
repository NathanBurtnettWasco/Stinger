...
class CalibrationRunPage(QWizardPage):
    ...
    def _on_point_measured(self, result) -> None:
        ...
        self.summary_label.setText(
            f"Latest point: {result.point_index}/{result.point_total}\n"
            f"Target: {result.target_psia:.2f} psia\n"
            f"Mensor: {result.mensor_psia if result.mensor_psia is not None else '--'}\n"
            f"Alicat: {result.alicat_psia if result.alicat_psia is not None else '--'}"
        )
